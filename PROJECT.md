...
The second matters more than the first, and it is the one piece still outstanding.
...
**Tests**: 12/12 headless physics + layout checks passing
...
### Phase 6 — Content *(blocked on you; nothing else can substitute)*

1. **Write 13 `role` lines.** Each entry in `projects.js` carries a `cvHint` with the
   CV bullets that most plausibly apply. One concrete sentence each about what you
   built. **This is the single highest-value task in the project.**
...
- **Test on a real phone.** Touch controls and safe-area insets are written but
  verified only on desktop Chrome. Highest-risk untested area.
- Fill the corridor's dead space in top-down view — an outer apron floor, or a
  tighter camera in narrow spaces.
- Sprint on shift (one speed setting today).
...
npm test          # 12 headless physics + layout checks
npm run build     # -> dist/
npm run classic   # regenerate classic.html only
```

**Controls** — WASD / arrows to move · drag to orbit · space to jump · **E** at a
kiosk. Touch: left half walks, right half looks. Gamepad: sticks, A jumps, X opens.
...
| `physics-smoke.mjs` | The 12 checks. Drives the real layout through Rapier in Node |
| `scripts/build-classic.mjs` | Generates `classic.html` |
